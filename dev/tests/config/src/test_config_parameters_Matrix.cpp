...
TEST_CASE("Test flash::config::parameters::Matrix class for int type") {
    ...
    defaultMatrix << 11, 12, 13, 14, 15, 16, 17, 18, 19;
    ...
    REQUIRE_THROWS_AS(tooManyCols.load(configFile), std::runtime_error);
    ...
    REQUIRE_THROWS_AS(tooManyRows.load(configFile), std::runtime_error);
    ...
    REQUIRE_THROWS_AS(vectorType.load(configFile), std::runtime_error);
    ...
    REQUIRE_THROWS_AS(oneRowIsTooShort.load(configFile), std::runtime_error);
}