...
// 3.  Data must be passed as `const` reference to write functions
// 4.  Data must be passed as reference to read functions
...
namespace hdf5 {
    ...
    /**
     * @namespace electrons
     * @brief Provides functions for reading and writing electron-related data to HDF5.
     *
     * This namespace contains utilities to serialize and deserialize electron data
     * to and from HDF5 files or groups. The data includes:
     * - Number of k-points
     * - Number of bands
     * - Electron energies
     * - Occupation numbers
     *
     * The `write` function stores the data in a structured HDF5 group, while the `read`
     * function loads it back into C++ variables and containers.
     */
    namespace electrons{
        /**
        ...
        void write(
            ...
            const index_t& numElectrons,
            const vector_t<real_t>& electrons,
         ...
         */
        void read(
            const H5::H5Object &h5object,
            index_t& numKpoints,
            index_t& numElectrons,
            vector_t<real_t>& electrons,
