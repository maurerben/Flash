...
/** @brief Validate Members of ElectronData. */
void validateMembers(const index_t numKpoints, const index_t numBands, const vector_t<real_t>& energies,
                     const vector_t<real_t>& occupations) {
    auto expectedSize = numKpoints * numBands;
    if (expectedSize != energies.size() || expectedSize != occupations.size()) {
        throw std::invalid_argument(
            "The sizes of energies and occupations must be the \
			same as `numKpoints * numBands.`");
    } else if ((occupations.array() < 0.0).any() || (occupations.array() > 2.0).any()) {
        throw std::invalid_argument("Occupations must be in the interval [0., 2.].");
    }
}
 ...
 */
ElectronData::ElectronData() : energies(), occupations() {}
 ...
 */
ElectronData::ElectronData(const index_t numKpoints, const index_t numBands, const vector_t<real_t>& energies,
                           const vector_t<real_t>& occupations)
    : numKpoints(numKpoints), numBands(numBands), energies{energies}, occupations{occupations} {
 ...
 */
ElectronData::ElectronData(const H5::H5Object& h5obj) {
    readElectrons(h5obj, numKpoints, numBands, energies, occupations);
    validateMembers(numKpoints, numBands, energies, occupations);
}
 ...
 */
const index_t ElectronData::NumKpoints() const { return numKpoints; }

/**
 * @brief Retrieve number of bands.
 */
const index_t ElectronData::NumBands() const { return numBands; }
 ...
 */
const real_t ElectronData::Energy(const ElectronIndex& index) const { return energies(index.getElectron()); }
 ...
 */
const vector_t<real_t> ElectronData::Energies(ElectronIndices indices) const {
 ...
 */
const vector_t<real_t>& ElectronData::Energies() const { return energies; }
 ...
 */
const real_t ElectronData::Occupation(const ElectronIndex& index) const { return occupations(index.getElectron()); }
 ...
 */
const vector_t<real_t> ElectronData::Occupations(ElectronIndices indices) const {
 ...
 */
const vector_t<real_t>& ElectronData::Occupations() const { return occupations; }
