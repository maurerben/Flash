...
class Matrix : public Parameter<Eigen::Matrix<T, Rows, Cols>> {
   public:
    /// @brief Initialze with Matrix#key
    /// @param[in] key
    Matrix(const std::string& key) : Parameter<Eigen::Matrix<T, Rows, Cols>>(key) {}

    /// @brief Initilize with Matrix#key and Parameter#defaultValue
    /// @param[in] key
    /// @param[in] defaultValue
    Matrix(const std::string& key, const Eigen::Matrix<T, Rows, Cols>& defaultValue)
        : Parameter<Eigen::Matrix<T, Rows, Cols>>(key, defaultValue) {}

    /**
     * @brief Setup Parameter#value
     * @details If \p node has no key Parameter#key, Parameter#value is set to Parameter#defaultValue
     * @param[in] node contains configuration
     * @throws runtime_error if \p node has no key Parameter#key and Parameter#defaultValue is not set or it does
     * not contain Cols elements
     */
    void load(const YAML::Node& node) {
        if (node[this->key]) {
            try {
                this->value = parseMatrixFromNode(node[this->key]);
   ...
   private:
    Eigen::Matrix<T, Rows, Cols> parseMatrixFromNode(const YAML::Node& matrixNode) {
        if (matrixNode.size() != Rows) {
            throw std::runtime_error(this->key + " has a not " + std::to_string(Rows) + " rows but " +
                                     std::to_string(matrixNode.size()) + ".");
        }
        Eigen::Matrix<T, Rows, Cols> matrix;
        for (std::size_t row = 0; row < Rows; ++row) {
            if (matrixNode[row].size() != Cols) {
                throw std::runtime_error(this->key + "[" + std::to_string(row) + "]" + " has a not " +
                                         std::to_string(Cols) + " cols but " + std::to_string(matrixNode[row].size()) +
                                         ".");
            }
            if (matrixNode[row].IsScalar()) {
                throw std::runtime_error(this->key + "[" + std::to_string(row) + "]" +
            ...
            for (std::size_t col = 0; col < Cols; ++col) {
                try {
                    ...
                    throw std::runtime_error(this->key + "[" + std::to_string(row) + ", " + std::to_string(col) +
                                             "] could not be loaded.");
                }
            }
        }
